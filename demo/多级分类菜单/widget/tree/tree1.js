/*
    Tree Widget for Internet Explorer (version 1.3.13)
...
function Tree(config)
{
    ...
    // root node
    rootNode = this.rootNode = new TreeNode();
...
*/
    ...
    destructor: function()
    {
        ...
        {
            var rootNav = this.rootNav, dataSource = this.dataSource, navObj;
            ...
            this.doc = this.funcTimeout = null;
...
//alert("blurOnEditbox()");
    ...
    nodeObj.removeTextStyle("treeEditbox");
    ...
    {
        nodeObj.setText(keyCode == 27 ? nodeObj.text : this.innerText);  // save changes if the "ESC" key is not pressed
    }
}
...
//alert("dragStartOnNode()");
    var nodeObj = this.nodeObj, treeObj = nodeObj.treeObj;
    nodeObj.select();  // select this node before dragging
    if (treeObj.fireEvent("dragstart", true, [nodeObj]))
    {
        var doc = treeObj.doc, body = doc.body, droppableSibling = Tree.droppableSibling, dragHint = Tree.dragHint;
        nodeObj.addTextStyle("treeDraggedText");
        ...
        if (! droppableSibling)
        {
            ...
            treeObj.rootNav.ondragover = Tree.dragEndOnSibling;
        }
        if (! dragHint)
        {
...
Tree.draggingOnNode = function(body)
{
    var dragStyle = Tree.dragHint.style;
    if (! body)
    {
        body = this.nodeObj.treeObj.doc.body;
    }
...
//alert("dragOverOnNode()");
    var tgtNodeObj = this.nodeObj, tgtTree = tgtNodeObj.treeObj, dropEffect = tgtNodeObj.setDropEffect(), funcTimeout;
    if (dropEffect != "none")
    {
        tgtNodeObj.addTextStyle("treeDroppableText");
        funcTimeout = tgtTree.funcTimeout;
...
//alert("Tree.dropNode(" + tgtNodeObj.text + "," + position + ")");
    ...
    Tree.dragEndOnNode();  // invoke the dragging-end function explicitly (the ondragend event is not triggered by IE if the source element is removed)
}

Tree.dropOnNode = function()
{
//alert("dropOnNode()");
    this.nodeObj.removeTextStyle("treeDroppableText");
    var tgtNodeObj = this.nodeObj;
    leaflife.cancelEvent();
    Tree.dropNode(Tree.nodeDragged, tgtNodeObj, "lastChild", tgtNodeObj.setDropEffect());
}

Tree.dragEndOnNode = function()
{
    var nodeDragged = Tree.nodeDragged, dragHint = Tree.dragHint;
    if (nodeDragged)
    {
        if (dragHint)
        {
...
{
    var tgtNodeObj = this.firstChild.nodeObj, posPreviousSibling = event.offsetY < tgtNodeObj.linkObj.offsetTop, dropEffect = tgtNodeObj.setDropEffect(posPreviousSibling ? "previousSibling" : "nextSibling"), droppableSibling = Tree.droppableSibling;
    if (dropEffect != "none")
    {
        tgtNodeObj.treeObj.rootNav.insertBefore(droppableSibling, posPreviousSibling ? tgtNodeObj.navObj : tgtNodeObj.navObj.nextSibling);
        droppableSibling.dropEffect = dropEffect;
...
Tree.dropOnSibling = function()
{
    ...
    Tree.dropNode(Tree.nodeDragged, tgtNodeObj, mode, tgtNodeObj.setDropEffect(mode));
}
...
Tree.dropOnHint = function()
{
    ...
    leaflife.cancelEvent();
    Tree.dropNode(Tree.nodeDragged, tgtNodeObj, mode, tgtNodeObj.setDropEffect());
}
...
*/
    ...
    draw: function(insertAtObj)
    {
        ...
        tbObj = this.navObj = treeObj.rootNav.insertBefore(doc.createElement("table"), insertAtObj);
...
//        alert(this.text + ".redraw(" + render + ")");
        ...
        if (parentNodeObj)
        {
            ...
            else if (render && parentNodeObj.nChildren > 1)  // draw node if sibling node exists
            {
                if (this.isLastNode)
                {
        ...
        if (nodeObj.treeObj.hierarchyVisible)
        {
            ...
            while (nodeObj)
            {
                for (i = nodeObj.nChildren - 1; i > -1; -- i)
                {
                    ...
                    childNode.navObj.cells[pos].className = code ? "treeImgLine4" : "";
                    nodes[len ++] = childNode;
                }
                nodeObj = nodes[-- len];
            }
...
//alert(this.text + ", " + checkIdObj.name);
                nodeChecked.put(checkIdObj.name, this);  // <parent node name, node>
            }
            else  // unchecked
            {
...
//        alert(this + ".detachFromParent()");
    ...
    reload: function()  // public
    {
        if (this.readyState == 1)
        {
            ...
            this.checkAncestors();
            var treeObj = this.treeObj;
            treeObj.fireEvent("beforeload", true, [this == treeObj.rootNode ? null : this]);  // Event: loading children nodes
        }
    },
    ...
    {
        var treeObj = this.treeObj, linkObj = this.linkObj, stateChange = linkObj.contentEditable != "true", range;
        if (linkObj && stateChange && treeObj.fireEvent("beforeedit", true, [this]))
        {
            ...
            this.addTextStyle("treeEditbox");
            linkObj.contentEditable = "true";
            range = treeObj.doc.body.createTextRange();
            range.moveToElementText(linkObj);
            range.select();
            linkObj.focus();
        }
    },
            ...
            while (nodeObj)
            {
                if ((expandChildren || expanding) && nodeObj.waitLoading(nodeObj.expand, nodeObj, expandChildren))
                {
                    for (i = nodeObj.nChildren - 1; i > -1; -- i)
                    {
                        ...
                        {
                            childNode.draw(treeObj.rootNav.firstChild);
                        }
    ...
    select: function(preserveSelected)
    {
        ...
        if (treeObj)
        {
            ...
            if (! nodeSelected.containsKey(nodeId) && treeObj.fireEvent("beforeselectchange", true, [this, true]) && ! nodeSelected.containsKey(nodeId))
            {
                ...
                this.setIconStyle();
                var treeNavObj = treeObj.rootNav, navObj = this.navObj, scrollTop = treeNavObj.scrollTop, scrollHeight = scrollTop + treeNavObj.clientHeight;
...
//        alert(this.text + ".copyTo(" + tgtNodeObj + "," + mode + ")");
    ...
    setDropEffect: function(position)
    {
        var srcNodeObj = Tree.nodeDragged, tgtTreeObj = this.treeObj, dropEffect = tgtTreeObj.dropEffect, srcTreeObj;
        leaflife.cancelEvent();
        if (srcNodeObj)
        {
            ...
            if (dropEffect != "none")
            {
                dropEffect = srcTreeObj.fireEvent("dragging", dropEffect, [srcNodeObj, tgtTreeObj, this, position == undefined ? "lastChild" : position]);
            }
            if (dropEffect == "none")
            {
                Tree.dragEndOnSibling();
